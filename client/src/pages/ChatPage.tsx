...
import axios from 'axios'
import type { Socket } from 'socket.io-client'
import { usePrivacyBlur } from '../helpers/Useprivacyblur'
...
export function ChatPage({ username, userId, onLogout, socket }: ChatPageProps) {
  ...
  const [messages, setMessages] = useState<Message[]>([])
  const [newMessage, setNewMessage] = useState('')
  const { isBlurred, reveal, quickBlur } = usePrivacyBlur()
    ...
    <div className="chat-shell">
      ...
      <main className="chat-pane">
        ...
        <div className="message-list-wrap">
          <section className={`message-list ${isBlurred ? 'blurred' : ''}`}>
            {messages.map((message) => (
              <div
                key={message._id}
                className={`message-row ${message.senderId === currentContact?._id ? 'incoming' : 'outgoing'}`}
          ...
          {isBlurred && (
            <button type="button" className="privacy-overlay" onClick={reveal}>
              Tap to reveal
            </button>
          )}
        </div>
